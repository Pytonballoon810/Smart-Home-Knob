  ...
  while (1)
  {
    ...
    if (!sk_demo_mode)
    {
      if (num_positions > 1)
      {
        ...
        uint16_t fill_color = hueToColor(spr_, state.config.hue? state.current_position : state.config.led_hue);
...
uint16_t DisplayTask::hueToColor(TFT_eSprite &spr_, int hue)
{ // Changed to reference
  // Convert HSV to RGB (S=100%, V=50%)
  float c = 1;
  ...
  return spr_.color565((r + m) * 255, (g + m) * 255, (b + m) * 255);
}
