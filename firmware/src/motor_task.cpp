...
// Modify momentum constants for much longer spinning
...
void MotorTask::run()
{
    ...
    float momentum_velocity = 0;
    float smoothed_velocity = 0;
    bool in_momentum_mode = false;
    ...
    while (1)
    {
        ...
        else
        {
            ...
            if (in_momentum_mode)
            {
                momentum_velocity *= MOMENTUM_DECAY_RATE;
                if (fabsf(momentum_velocity) < MOMENTUM_MIN_VELOCITY)
                {
                    in_momentum_mode = false;
                    momentum_velocity = 0;
                }
                ...
                {
                    // Directly move the motor based on momentum
                    float motor_speed = momentum_velocity; // Scale factor for motor speed
